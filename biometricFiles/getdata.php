                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if ($row = mysqli_fetch_assoc($resultl)){
                        ...
                        if ($row['username'] != "None" && $row['add_fingerid'] == 0){
                         ...
                         //   $sql = "SELECT * FROM users_logs WHERE fingerprint_id=? AND votestatus=1";
                         $sql = "SELECT * FROM users_logs WHERE fingerprint_id=?";
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                if (!$row = mysqli_fetch_assoc($resultl)){
                                    ...
                                    $sql = "INSERT INTO users_logs (username, serialnumber, fingerprint_id, device_uid, device_dep,checkindate, timein, votestatus) VALUES (? ,?, ?, ?, ?, ?, ?, 1)";
                                    $result = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result, $sql)) {
                                        ...
                                        $timeout = "00:00:00";
                                        mysqli_stmt_bind_param($result, "sdissss", $Uname, $Number, $fingerID, $device_uid, $device_dep, $d, $t);
                                        mysqli_stmt_execute($result);
                                        
                                        $sql="UPDATE users SET votestatus=1 WHERE fingerprint_id=?";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
