                        ...
                        if (!empty($password)) {
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                if (!$row = mysqli_fetch_assoc($resultl)) {
                                    ...
                                    $sql="UPDATE users SET username=?, serialnumber=?, email=?, user_pwd=?, gender=?, user_date=CURDATE() WHERE fingerprint_select=1 AND device_uid=?";
                                    $result = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result, $sql)) {
                                        ...
                                        mysqli_stmt_bind_param($result, "sdssss", $Uname, $Number, $Email, $newPwdHash, $Gender, $dev_uid );
                                        mysqli_stmt_execute($result);
...
if (isset($_POST['vote'])) {
    ...
    //check if the user own the finger id
    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            if ($row['votestatus'] == 1) {
                $sql="UPDATE users SET votestatus=2 WHERE fingerprint_id=?";
                $result = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($result, $sql)) {
                    echo "SQL_Error_select_Fingerprint";
                    exit();
                    ...
                    mysqli_stmt_bind_param($result, "s", $finger_id );
                    mysqli_stmt_execute($result);
                    $sql="UPDATE users_logs SET votestatus=2 WHERE fingerprint_id=?";
                    $result = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        echo "SQL_Error_select_Fingerprint";
                        exit();
            ...
            else if ($row['votestatus'] == 0) {
                echo "Scan your fingerprint";
                exit();
            }
            else {
                echo "Already Voted";
...
if (isset($_GET['reset'])) {
    ...
    $sql="UPDATE users SET votestatus=0 WHERE fingerprint_id=?";
    $result = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($result, $sql)) {
        echo "SQL_Error_select_Fingerprint";
        exit();
        ...
        $sql="UPDATE users_logs SET votestatus=0 WHERE fingerprint_id=?";
        $result = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($result, $sql)) {
            echo "SQL_Error_select_Fingerprint";
            exit();
            ...
            echo "User vote has been resetted";
            exit();
        }
    }
}
